...
public class ProductControllers {
    ...
    @GetMapping
    public ResponseEntity<Page<ProductDTO>> getAllProducts(@RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "id") String sortBy){
        Pageable requestedPage = PageRequest.of(page, 10, Sort.by(Sort.Direction.ASC, sortBy, "price"));

        Page<ProductDTO> productDTOs =  productService.findAll(requestedPage);
